...
import {
  getPopularMovies,
  getBestComedies,
  getActionandAdventure,
  getVideos,
} from "../config.js";

import PopularMovies from "../views/PopularMovies";
import BestComedies from "../views/BestComedies";
import ActionAndAdventure from "../views/ActionAndAdventure";
...
class Main {
  ...
  activeFunctions() {
    ...
    getPopularMovies().then((data) => {
      const { results } = data;
      PopularMovies.callFunctions();
      PopularMovies.render(results);
    });

    getBestComedies().then((data) => {
      const { results } = data;
      BestComedies.callFunctions();
      BestComedies.render(results);
    });

    getActionandAdventure().then((data) => {
      const { results } = data;
      ActionAndAdventure.callFunctions();
      ActionAndAdventure.render(results);
    });
  }
  ...
  render() {
    ...
    <header>
      ...
      <main>
          <section>
              <div class="container">
                ${SectionHeader.render("Continue Watching")}
                ${ViewAll.render("#")}
                ${Videos.render()}
              </div>
          </section>
          ...
          <section> 
             ...
             ${SectionHeader.render("Popular Movies 2022")}
             ${ViewAll.render("#")}
              </div>
          ...
          <section> 
             ...
             ${SectionHeader.render("Best Comedies")}
             ${ViewAll.render("#")}
              </div>
          ...
          <section> 
             ...
             ${SectionHeader.render("Action & Adventure")}
             ${ViewAll.render("#")}
              </div>
